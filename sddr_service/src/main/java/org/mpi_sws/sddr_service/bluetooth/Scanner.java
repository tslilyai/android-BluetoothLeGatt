...
import org.mpi_sws.sddr_service.SDDR_Native;
...
public class Scanner {
    private static final String TAG = "SDDR: " + Scanner.class.getSimpleName();
    ...
    /**
     * Filter our scans so we only discover SDDR devices
     */
    ...
    private class SDDRScanCallback extends ScanCallback {
        ...
        private void processResult(ScanResult result) {
            ...
            if (record == null) {
                ...
                if (datamap.size() != 1) {
                    Log.d(TAG, "Not SDDR: More than one service");
                    return;
                }
                ...
                for (ParcelUuid pu : datamap.keySet()) {
                    ...
                    if (len != Constants.PUUID_LENGTH + Constants.ADVERT_LENGTH) {
                        Log.d(TAG,"Not SDDR: Wrong advert length " + len);
                        return;
                    }
                    ...
                    Log.d(TAG, "Processing SDDR scanresult with data " + Utils.getHexString(datahead) + Utils.getHexString(datatail) + ":\n"
    ...
    private class GattClientCallback extends BluetoothGattCallback {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (status == BluetoothGatt.GATT_FAILURE) {
                Log.d(TAG, "Failure to connect to device");
