...
JNIEXPORT void JNICALL Java_org_mpi_1sws_sddr_1userspace_SDDR_1Native_c_1mallocRadio
    ...
    LOG_D("c_SDDR", "-- set up radio pointer %p", radioPtr);
   
    // initialize all the JNI IDs we will need
    jni_cache.clsRA = (jclass) env->NewGlobalRef(env->FindClass("org/mpi_sws/sddr_userspace/SDDR_Core$RadioAction"));
    jni_cache.clsAT = (jclass) env->NewGlobalRef(env->FindClass("org/mpi_sws/sddr_userspace/SDDR_Core$RadioAction$actionType"));
    jni_cache.clsArray = (jclass) env->NewGlobalRef(env->FindClass("java/util/ArrayList"));
    jni_cache.clsNative = (jclass) env->NewGlobalRef(env->FindClass("org/mpi_sws/sddr_userspace/SDDR_Native"));
    if (!jni_cache.clsAT || !jni_cache.clsRA || !jni_cache.clsArray) {
        LOG_D("c_SDDR", "Class not found");
        assert(0);
    }
    jni_cache.fidChangeEpoch = env->GetStaticFieldID(jni_cache.clsAT, "ChangeEpoch", "Lorg/mpi_sws/sddr_userspace/SDDR_Core$RadioAction$actionType;");
    jni_cache.fidDiscover = env->GetStaticFieldID(jni_cache.clsAT, "Discover", "Lorg/mpi_sws/sddr_userspace/SDDR_Core$RadioAction$actionType;");
    ...
    jni_cache.constructorRA = env->GetMethodID(jni_cache.clsRA, "<init>", 
                "(Lorg/mpi_sws/sddr_userspace/SDDR_Core$RadioAction$actionType;J)V");
    ...
    if (!jni_cache.fidChangeEpoch || !jni_cache.fidDiscover || !jni_cache.addArray ||
        ...
        LOG_D("c_SDDR", "Method or FieldID not found");
        assert(0);
    }
...
JNIEXPORT jbyteArray JNICALL Java_org_mpi_1sws_sddr_1userspace_SDDR_1Native_c_1getRandomAddr
  (JNIEnv *env, jobject obj) {
    ...
    LOG_D("c_SDDR", "Got Addr %s with len %d", addr.toString().c_str(), addr.len());
 ...
 */
 JNIEXPORT void JNICALL Java_org_mpi_1sws_sddr_1userspace_SDDR_1Native_c_1postDiscovery
  (JNIEnv *env, jobject obj) {
    ...
    jobject arraylistObj = env->GetStaticObjectField(jni_cache.clsNative, 
            jni_cache.fidc_EncounterMsgs); 
    jbyteArray bytes;
...
JNIEXPORT jbyteArray JNICALL Java_org_mpi_1sws_sddr_1userspace_SDDR_1Native_c_1getRetroactiveMatches
        (JNIEnv *env, jobject obj, jbyteArray arr)
{
    jsize len = env->GetArrayLength(arr);
    jbyte* msg = env->GetByteArrayElements(arr,0);
